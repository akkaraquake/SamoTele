...
function exercise_onload(first_data, length) {
    ...
    // Устанавливаем первый перевод
    word.innerHTML = first_data['word_translate'];

    // Устанавливаем слово (правильный ответ)
    right_translate.innerHTML = first_data['word'];
...
function go_next(count_answers) {
    ...
    word.innerHTML = words[count_answers - 1]['word_translate'];
    
    right_translate.innerHTML = words[count_answers - 1]['word'];
...
function show_results(data, rows) {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        if (answer_status_list[i]) {
            input_field_1.style.color = "#01ee60";
        }
        else {
            input_field_1.style.color = "red";
            input_field_1.title = "Ваш ответ: " + user_answer_list[i];
        }
